...
bool printPageItemSelected(uint16_t index)
{
  ...
  else if (index < infoFile.folderCount + infoFile.fileCount)  // gcode file
  {
    ...
    else
    {
      ...
      setPrintModelIcon(infoFile.source < ONBOARD_MEDIA && model_DecodeToFlash(infoFile.path));
...
void menuPrintFromSource(void)
{
  ...
  {
    if (infoFile.source == ONBOARD_MEDIA)  // error when the filesystem selected from TFT media not available
