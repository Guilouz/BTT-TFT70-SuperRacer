...
{ // searches an expression in the whole dmaL2Cache
...
{// searches an expression in dmaL2Cache from where a previous successful search left off
  uint16_t str_len = strlen(str);
  char* pStr_tmp;
...
void hostActionCommands(void)
{
  if (ack_seen(":notification "))
  {
    ...
    else
    {
      ...
      if (!ack_seen("prête."))  // avoid to display unneeded/frequent useless notifications (e.g. "My printer Ready.")
      {
